    ...
    <div class="container pt-5">
        <nav class="nav nav-tabs flex-column flex-sm-row mt-1">
            @foreach ($pages as $key => $page)
                {{-- @if ($page == $page->first()) --}}
                <a class="flex-sm-fill text-sm-center fw-bold nav-link {{ $loop->iteration == 1 ? 'active' : '' }}"
                    data-bs-toggle="tab" data-bs-target="#{{ $page->slug }}">{{ $page->title }}</a>
                {{-- @else --}}
        ...
        <div class="tab-content mb-5" id="myTabContent">
            @foreach ($pages as $key => $page)
                <div class="tab-pane fade show {{ $loop->iteration == 1 ? 'active' : '' }} " id="{{ $page->slug }}"
